...
const editorPaneSize = ref(100);
const bottomPaneSize = ref(0);
...
watch(isBottomPanelOpen, (isOpen) => {
  if (isOpen) {
    editorPaneSize.value = 70;
    bottomPaneSize.value = 30;
  } else {
    editorPaneSize.value = 100;
...
function handleFileClose(fileId: string) {
  ...
  if (activeFileId.value === fileId) {
    activeFileId.value =
      openFiles.value[index]?.id ?? openFiles.value[index - 1]?.id ?? null;
  }
}
      ...
      <Pane :size="80" class="h-full">
        <Splitpanes
          ...
          @resized="
            (panes) => {
              if (panes.length >= 2) {
                editorPaneSize = panes[0].size;
                bottomPaneSize = panes[1].size;

                if (panes[1].size < 2) {
                  isBottomPanelOpen = false;
                } else if (panes[1].size > 2 && !isBottomPanelOpen) {
