...
```typescript
type WorkerHttpHandler<T = any> = (request: Request, context: WorkerContext) => Promise<T> | T;
```
...
```typescript
type PolicyHandler = (requests: PolicyExecutionRequest[]) => boolean[] | Promise<boolean[]>;
```
...
<Tabs syncKey="sdk-install-troubleshoot">
<TabItem label="npm">
