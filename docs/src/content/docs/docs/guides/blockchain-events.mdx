...
Keep this Anvil instance running for the next guides; restarting Anvil resets the chain and wipes deployed contracts.
...
forge create --root . TestToken.sol:TestToken \
...
```
  ...
  <TabItem label="without jq">

```bash
npm run build
DEPLOY_OUTPUT=$(nxcc worker deploy --bundle workers/manifest.template.json)
echo "$DEPLOY_OUTPUT"
```

After the command runs, copy the value from the `workOrderId` field and set it manually:

```bash
export WORKER_ID="paste-work-order-id-here"
```
...
```diff workers/manifest.template.json
-    "chain": 31337,
-    "address": ["0x5FbDB2315678afecb367f032d93F642f64180aa3"],
-    "gateways": ["ws://host.docker.internal:8545"],
+    "chain": 1,
