...
export default {
  async fetch(request: Request): Promise<Response> {
    ...
    const handler = url.pathname.startsWith("/") ? url.pathname.substring(1) : url.pathname;
...
export default policy((requests) => {
  return requests.map((request) => {
    ...
    console.log(`Consumer bundle ${bundleHash}: ${isAllowed ? "ALLOW" : "DENY"}`);
    return isAllowed;
  });
});
```
...
export default policy((requests) => {
  return requests.map((request) => {
    ...
    if (!businessHours) {
      console.log(`Access denied outside business hours (current: ${hour}:00 UTC)`);
      return false;
    }
...
export default policy((requests) => {
  ...
  return requests.map((request, index) => {
    ...
    console.log(`[${requestId}] Consumer bundle:`, request.consumer?.bundle_hash);
