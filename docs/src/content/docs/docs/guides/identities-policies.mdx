...
import { createWalletClient, http, privateKeyToAccount, formatEther } from "viem";
...
# Deploy worker
nxcc worker deploy --bundle secure-manifest.json --rpc-url http://localhost:6922
```
...
export default policy((requests) => {
  return requests.map((request) => {
    ...
    const validSignatures = signatures.filter((sig) => approvedSigners.has(sig.signer));
