...
cleanup() {
	...
	if [ -n "${POSTBACK_PID:-}" ]; then
		kill "$POSTBACK_PID" 2>/dev/null
	fi
	if [ -n "${ZENROOM_VM_PID:-}" ]; then
		kill "$ZENROOM_VM_PID" 2>/dev/null
	fi
	if [ -n "${WORKERD_VM_PID:-}" ]; then
		kill "$WORKERD_VM_PID" 2>/dev/null
	fi
	if [ -n "${ENCLAVE_PID:-}" ]; then
		kill "$ENCLAVE_PID" 2>/dev/null
	fi
