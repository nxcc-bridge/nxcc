...
# Wait for nodes to definitely connect if they're going to
sleep 2
...
poll_until_secret_found "$alice_ENCLAVE_SOCK" "$SECRET_CHAIN_ID" "$SECRET_IDENTITY_ADDR" "$SECRET_IDENTITY_ID_NUM" 60 2 || exit 1 # 60s timeout, check every 2s
...
poll_until_secret_found "$bob_ENCLAVE_SOCK" "$SECRET_CHAIN_ID" "$SECRET_IDENTITY_ADDR" "$SECRET_IDENTITY_ID_NUM" 60 2 || exit 1
