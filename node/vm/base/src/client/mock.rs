...
impl VmClient for MockVmServiceClient {
    ...
    async fn get_attestation(
        ...
        use nxcc_interface::{gateway::BlockInfo, types::attestation::RawAttestation};
        ...
        match &*self.attestation_behavior.lock().unwrap() {
            MockAttestationBehavior::Standard => {
                ...
                // In the new approach, user_data is ignored since userdata is generated by the enclave
            ...
            MockAttestationBehavior::Custom(report) => {
                // Return the custom report (user_data is ignored in the new approach)
                Ok(report.clone())
            }
