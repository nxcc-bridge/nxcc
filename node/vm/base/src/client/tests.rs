            ...
            nxcc_interface::proto::vm::GetAttestationResponse {
                bundle: Some(nxcc_interface::proto::interface::AttestationBundle {
                    ...
                    detached_userdata: {
                        ...
                        // The req.user_data is ignored in the new approach since userdata is generated by the enclave
...
async fn test_client_operations() -> Result<(), Box<dyn Error>> {
    ...
    // Parse the detached userdata to check values
    let userdata =
        nxcc_attestation::user_data_binding::UserData::from_cbor(&attestation.detached_userdata)
            .unwrap();
    assert_eq!(userdata.ephemeral_public_key, vec![1, 2, 3, 4]); // Mock data
...
async fn test_mock_client() {
    use nxcc_interface::{
        ...
        types::attestation::{AttestationBundle, RawAttestation},
    };
    ...
    // Parse the detached userdata to check ephemeral key
    let userdata =
        nxcc_attestation::user_data_binding::UserData::from_cbor(&attestation.detached_userdata)
            .unwrap();
    ...
    // Create test userdata with ephemeral key and freshness info
    let test_userdata = nxcc_attestation::user_data_binding::UserData::new(
        vec![9, 8, 7], // ephemeral key (3 bytes for test)
        vec![nxcc_attestation::BlockInfo {
    ...
    // Parse the detached userdata to check the custom values
    let userdata =
        nxcc_attestation::user_data_binding::UserData::from_cbor(&attestation.detached_userdata)
            .unwrap();
    assert_eq!(userdata.ephemeral_public_key, vec![9, 8, 7]); // From custom attestation
    assert_eq!(
        userdata
            .block_hashes
            .iter()
